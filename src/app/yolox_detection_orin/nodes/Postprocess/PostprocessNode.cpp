...
void ApplyNmsForClass(
    ...
    for (int index = 0; index < valid_count; ++index) {
        ...
        if (selected == -1 || class_ids[selected] != filter_class_id) {
            continue;
        }

        for (int next = index + 1; next < valid_count; ++next) {
            const int candidate = order[next];
            if (candidate == -1 || class_ids[candidate] != filter_class_id) {
                continue;
            }
...
void PostprocessNode::execute(GryFlux::DataPacket& packet, GryFlux::Context& ctx) {
    ...
    for (size_t stride_index = 0; stride_index < strides_.size(); ++stride_index) {
        ...
        const float* reg_data =
            detect_packet.infer_outputs[stride_index * 3 + 0].data();
        const float* obj_data =
            detect_packet.infer_outputs[stride_index * 3 + 1].data();
        const float* cls_data =
            detect_packet.infer_outputs[stride_index * 3 + 2].data();
    ...
    const std::set<int> unique_class_ids(class_ids.begin(), class_ids.end());
    for (const int class_id : unique_class_ids) {
        ApplyNmsForClass(
            valid_count,
            filter_boxes,
            class_ids,
    ...
    for (int order_index = 0; order_index < valid_count; ++order_index) {
        ...
        float final_x1 =
            (static_cast<float>(Clamp(x1, 0, model_width_)) -
             static_cast<float>(detect_packet.preproc_data.x_offset)) /
            detect_packet.preproc_data.scale;
        float final_y1 =
            (static_cast<float>(Clamp(y1, 0, model_height_)) -
             static_cast<float>(detect_packet.preproc_data.y_offset)) /
            detect_packet.preproc_data.scale;
        float final_x2 =
        ...
        final_x1 = std::max(
            0.0f,
            std::min(final_x1,
                     static_cast<float>(detect_packet.preproc_data.original_width)));
        final_y1 = std::max(
            0.0f,
            std::min(final_y1,
                     static_cast<float>(detect_packet.preproc_data.original_height)));
        final_x2 = std::max(
            0.0f,
            std::min(final_x2,
                     static_cast<float>(detect_packet.preproc_data.original_width)));
        ...
        detect_packet.detections.push_back(
            Detection{final_x1,
                      final_y1,
                      final_x2,
                      final_y2,
