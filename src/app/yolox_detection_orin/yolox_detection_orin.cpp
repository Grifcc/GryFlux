...
void PrintUsage(const char* app_name) {
    std::cout
        ...
        << "  -m, --model <path>          TensorRT engine path (required)\n"
        ...
        << "      --max-active <num>      AsyncPipeline max active packets "
           "(default: 8)\n"
        << "      --npu-instances <num>   TensorRT execution slot count "
           "(default: 2)\n";
}
...
void InitializeLogger() {
    ...
    LOG.setAppName("yolox_detection_orin");
}
...
int RunPipeline(const AppOptions& options) {
    ...
    LOG.info("Starting yolox_detection_orin pipeline");
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        LOG.error("yolox_detection_orin failed: %s", exception.what());
        return 1;
    }
}
