...
constexpr size_t kMaxActivePackets = 2;
constexpr const char kProfilingTimelinePath[] = "realesrgan_timeline.json";
...
int RunPipeline(const AppOptions& options) {
    ...
    auto source = std::make_shared<RealEsrganSource>(options.input_dir);
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        LOG.error("super-resolution_realesrgan_Ascend failed: %s", exception.what());
        return 1;
    }
}
