...
void PrintUsage(const char* app_name) {
    std::cout
        ...
        << "      --model <path>           TensorRT engine path (required)\n"
        << "      --threads <num>          AsyncPipeline thread pool size (default: 8)\n"
        << "      --packets <num>          AsyncPipeline max active packets (default: 16)\n"
        << "      --npu-instances <num>    TensorRT execution slot count (default: 2)\n"
        << "      --device <num>           CUDA device id (default: 0)\n"
        << "      --width <num>            Dynamic model fallback width (default: 640)\n"
        << "      --height <num>           Dynamic model fallback height (default: 480)\n";
}
...
ParseAppOptionsResult ParseAppOptions(
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--npu-instances") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->npu_instance_count)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        ...
        if (argument == "--width") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->model_width)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        if (argument == "--height") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->model_height)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
...
void InitializeLogger() {
    ...
    LOG.setAppName("image_fusion_orin");
}
...
int RunPipeline(const AppOptions& options) {
    ...
    LOG.info(
        "Starting image_fusion_orin pipeline with model input %dx%d",
        runtime_resources.model_info.model_width,
        runtime_resources.model_info.model_height);
    ...
    const auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        end_time - start_time);
    LOG.info(
        "image_fusion_orin completed in %lld ms",
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        LOG.error("image_fusion_orin failed: %s", exception.what());
        return 1;
    }
}
