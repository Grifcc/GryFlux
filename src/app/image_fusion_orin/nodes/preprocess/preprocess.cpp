...
void PreprocessNode::execute(GryFlux::DataPacket &packet, GryFlux::Context &ctx) {
    ...
    // 1. 尺寸调整 (Resize)
    cv::resize(fusion_packet.vis_raw, fusion_packet.vis_resize, cv::Size(GetFusionModelWidth(), GetFusionModelHeight()));
    cv::resize(fusion_packet.ir_raw, fusion_packet.ir_resize, cv::Size(GetFusionModelWidth(), GetFusionModelHeight()));
