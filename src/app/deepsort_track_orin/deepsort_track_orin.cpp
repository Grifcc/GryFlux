...
void PrintUsage(const char* app_name) {
    std::cout
        ...
        << "  -y, --yolox <path>            YOLOX TensorRT engine path (required)\n"
        << "  -r, --reid <path>             ReID TensorRT engine path (required)\n"
        << "  -t, --threads <num>           AsyncPipeline thread pool size "
           "(default: 12)\n"
        << "  -p, --packets <num>           AsyncPipeline max active packets "
           "(default: 8)\n"
        << "      --det-width <num>         Detection model input width "
           "(default: 640)\n"
        << "      --det-height <num>        Detection model input height "
           "(default: 640)\n"
        << "      --reid-width <num>        ReID crop width (default: 128)\n"
        << "      --reid-height <num>       ReID crop height (default: 256)\n"
        << "      --reid-feature-dim <num>  ReID feature dimension "
           "(default: 512)\n"
        << "      --conf <float>            Detection confidence threshold "
           "(default: 0.3)\n"
        << "      --nms <float>             Detection NMS threshold "
           "(default: 0.45)\n"
        << "      --det-npu-instances <num> Detection TensorRT execution slot "
           "count (default: 2)\n"
        << "      --reid-npu-instances <num> ReID TensorRT execution slot "
           "count (default: 2)\n"
        << "  -d, --device <num>            CUDA device id (default: 0)\n";
}
...
bool ValidateOptions(const char* app_name, const AppOptions& options) {
    ...
    if (options.thread_pool_size == 0 || options.max_active_packets == 0 ||
        options.detection_instance_count == 0 ||
        options.reid_instance_count == 0) {
        std::cerr << app_name
                  << ": --threads, --packets, --det-npu-instances and "
                     "--reid-npu-instances must be greater than zero."
                  << std::endl;
        return false;
    }
...
ParseAppOptionsResult ParseAppOptions(
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "-t" || argument == "--threads") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->thread_pool_size)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        if (argument == "-p" || argument == "--packets") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->max_active_packets)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        if (argument == "--det-width") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->detection_model_width)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        if (argument == "--det-height") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->detection_model_height)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        if (argument == "--reid-width") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->reid_width)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        if (argument == "--reid-height") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->reid_height)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        if (argument == "--reid-feature-dim") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->reid_feature_dim)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        if (argument == "--conf") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->confidence_threshold)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        if (argument == "--nms") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->nms_threshold)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
        ...
        if (argument == "-d" || argument == "--device") {
            const char* value = require_value(argument);
            if (!value || !ParseScalarArgument(
                              app_name, argument, value, &options->device_id)) {
                return ParseAppOptionsResult::kExitFailure;
            }
            continue;
        }
...
void InitializeLogger() {
    ...
    LOG.setAppName("deepsort_track_orin");
}
...
int RunPipeline(const AppOptions& options) {
    ...
    LOG.info("Starting deepsort_track_orin pipeline");
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        LOG.error("deepsort_track_orin failed: %s", exception.what());
        return 1;
    }
}
