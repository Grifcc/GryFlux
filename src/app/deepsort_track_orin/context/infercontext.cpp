...
public:
    explicit SharedDetectionModel(const std::string& engine_model_path) {
...
private:
    void InspectBindings() {
        ...
        if (input_binding_index_ < 0 || output_binding_indices_.empty()) {
            throw std::runtime_error(
                "TensorRT engine does not expose the expected detection bindings");
        }
    }
...
void InferContext::copyToDevice(const void* host_data, size_t size) {
    if (size != input_buffer_size_) {
        throw std::runtime_error("DeepSORT detection input size does not match TensorRT engine");
    }
    CUDA_CHECK_THROW(
        cudaMemcpyAsync(
            input_buffer_,
        ...
        "Failed to copy detection input tensor to device");
}
...
void InferContext::executeModel() {
    TRT_CHECK_THROW(
        enqueueContext(
            ...
            bindings_,
            static_cast<cudaStream_t>(stream_)),
        "TensorRT detection inference failed");
}
    ...
    for (auto& out_buf : output_buffers_) {
        CUDA_CHECK_THROW(
            cudaMemcpyAsync(
                ...
                static_cast<cudaStream_t>(stream_)),
            "Failed to copy detection output tensor to host");
    }
    CUDA_CHECK_THROW(
        cudaStreamSynchronize(static_cast<cudaStream_t>(stream_)),
        "Failed to synchronize detection CUDA stream");
}
...
void InferContext::allocateBuffers() {
    ...
    CUDA_CHECK_THROW(
        ...
        "Failed to allocate detection input buffer");
    ...
    for (size_t index = 0; index < output_binding_indices.size(); ++index) {
        ...
        CUDA_CHECK_THROW(
            ...
            "Failed to allocate detection output device buffer");
        CUDA_CHECK_THROW(
            cudaMallocHost(&output.host_buffer, output.size),
            "Failed to allocate detection output host buffer");
    ...
    if (input_index_ < 0 || output_buffers_.empty()) {
        throw std::runtime_error(
            "TensorRT engine bindings do not match deepsort_track_orin detection requirements");
    }
}
...
std::vector<std::shared_ptr<GryFlux::Context>> CreateDetectionInferContexts(
    ...
    if (instance_count == 0) {
        throw std::runtime_error("Detection context instance count must be greater than zero");
    }

    auto shared_model = std::make_shared<SharedDetectionModel>(engine_model_path);
