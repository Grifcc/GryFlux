...
void FeatureTensor::doInference(vector<cv::Mat>& imgMats, DETECTIONS& det) {
    ...
    for (int i = 0;i < imgMats.size();i++){
        ...
        [[maybe_unused]] float npu_performance = cal_NPU_performance(history_time, sum_time, cost_time / 1.0e3);
        // printf("Deepsort: %f NPU(%d) performance : %f\n", current_time_millis()/1000, _cpu_id, npu_performance);
	}
}
...
bool FeatureTensor::getRectsFeature(const cv::Mat& img, DETECTIONS& det) {
    ...
    double timeBeforeGetRectsFeature = current_time_millis();
    ...
    double timeAfterGetRectsFeature = current_time_millis();
