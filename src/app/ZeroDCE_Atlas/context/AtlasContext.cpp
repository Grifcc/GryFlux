...
AtlasContext::AtlasContext(Config config)
...
size_t AtlasContext::getInputBufferSize(size_t index) const {
...
size_t AtlasContext::getInputBufferSize() const {
    return input_buffers_.empty() ? 0 : input_buffers_[0].size;
}

void AtlasContext::copyToDevice(const void* host_data, size_t size) {
    copyToDevice(0, host_data, size);
}

void AtlasContext::copyToDevice(size_t input_index, const void* host_data, size_t size) {
...
void AtlasContext::executeModel() {
    ThrowIfAclError(aclrtSetDevice(device_id_), "aclrtSetDevice");
    ThrowIfAclError(aclmdlExecute(model_id_, input_dataset_, output_dataset_),
                    "aclmdlExecute");
}

void AtlasContext::copyToHost() {
    ThrowIfAclError(aclrtSetDevice(device_id_), "aclrtSetDevice");
    for (auto& output : output_buffers_) {
        ThrowIfAclError(aclrtMemcpy(output.host_buffer,
                                    output.size,
...
void AtlasContext::copyToHost(size_t output_index, void* host_buffer, size_t size) {
    ...
    ThrowIfAclError(aclrtSetDevice(device_id_), "aclrtSetDevice");
    ThrowIfAclError(aclrtMemcpy(host_buffer,
                                size,
                                output.device_buffer,
                                size,
                                ACL_MEMCPY_DEVICE_TO_HOST),
...
void* AtlasContext::getOutputHostBuffer(size_t index) const {
...
size_t AtlasContext::getOutputSize(size_t index) const {
    ...
    return output_buffers_[index].size;
}
...
std::vector<std::shared_ptr<GryFlux::Context>> CreateAtlasContexts(
    ...
    for (size_t index = 0; index < instance_count; ++index) {
        auto context = std::make_shared<AtlasContext>(
            AtlasContext::Config{om_model_path, device_id});
