...
void PreprocessNode::execute(GryFlux::DataPacket& packet, GryFlux::Context& ctx) {
    ...
    cv::Mat rgb_image;
    cv::cvtColor(track_packet.original_image, rgb_image, cv::COLOR_BGR2RGB);

    const int image_width = rgb_image.cols;
    const int image_height = rgb_image.rows;
    track_packet.preproc_data.original_width = image_width;
    track_packet.preproc_data.original_height = image_height;
    ...
    if (image_width == model_width_ && image_height == model_height_) {
        ...
        track_packet.preproc_data.scale = 1.0f;
        track_packet.preproc_data.x_offset = 0;
        track_packet.preproc_data.y_offset = 0;
    } else {
        ...
        track_packet.preproc_data.scale = scale;
        ...
        track_packet.preproc_data.x_offset = (model_width_ - resized_width) / 2;
        track_packet.preproc_data.y_offset = (model_height_ - resized_height) / 2;

        resized_image.copyTo(
            letterbox_image(cv::Rect(track_packet.preproc_data.x_offset,
                                     track_packet.preproc_data.y_offset,
                                     resized_width,
                                     resized_height)));
    ...
    float* nchw_buffer = track_packet.preproc_data.nchw_data.data();
