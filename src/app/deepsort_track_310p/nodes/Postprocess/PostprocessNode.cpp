...
void ApplyGlobalNms(
    ...
    for (int first = 0; first < valid_count; ++first) {
        ...
        for (int second = first + 1; second < valid_count; ++second) {
            ...
            const float xmin0 = output_locations[selected_index * 4 + 0];
            const float ymin0 = output_locations[selected_index * 4 + 1];
            const float xmax0 = xmin0 + output_locations[selected_index * 4 + 2];
            const float ymax0 = ymin0 + output_locations[selected_index * 4 + 3];

            const float xmin1 = output_locations[candidate_index * 4 + 0];
            const float ymin1 = output_locations[candidate_index * 4 + 1];
            const float xmax1 = xmin1 + output_locations[candidate_index * 4 + 2];
            const float ymax1 = ymin1 + output_locations[candidate_index * 4 + 3];
...
void PostprocessNode::execute(GryFlux::DataPacket& packet, GryFlux::Context& ctx) {
    ...
    for (int order_index = 0; order_index < valid_count; ++order_index) {
        ...
        if (track_packet.detections.size() >= track_packet.max_detection_capacity) {
            LOG.warning(
                "[PostprocessNode] Packet %d reached detection capacity %zu, remaining detections are dropped",
                track_packet.frame_id,
        ...
        final_x1 = std::clamp(final_x1, 0.0f,
                              static_cast<float>(track_packet.preproc_data.original_width));
        final_y1 = std::clamp(final_y1, 0.0f,
                              static_cast<float>(track_packet.preproc_data.original_height));
        final_x2 = std::clamp(final_x2, 0.0f,
                              static_cast<float>(track_packet.preproc_data.original_width));
        final_y2 = std::clamp(final_y2, 0.0f,
                              static_cast<float>(track_packet.preproc_data.original_height));

        Detection detection;
        detection.x1 = final_x1;
        detection.y1 = final_y1;
        detection.x2 = final_x2;
        detection.y2 = final_y2;
        detection.score = object_probabilities[order_index];
        detection.class_id = class_ids[detection_index];
        track_packet.detections.push_back(detection);
    }
}
