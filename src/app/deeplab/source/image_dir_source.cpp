...
DeeplabImageDirSource::DeeplabImageDirSource(const std::string &datasetDir)
    : datasetDir_(datasetDir)
{
    ...
    LOG.info("DeeplabImageDirSource initialized: dir=%s files=%zu",
             datasetDir_.string().c_str(),
             imageFiles_.size());
}

std::unique_ptr<GryFlux::DataPacket> DeeplabImageDirSource::produce()
{
...
bool DeeplabImageDirSource::isSupportedImage(const fs::path &path)
{
